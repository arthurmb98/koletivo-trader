...
class Mt5Broker(Broker):
    ...
    def send(self, signal: Signal, volume: float) -> dict[str, Any]:
        return self.send_market(signal, volume)

    def send_market(self, signal: Signal, volume: float) -> dict[str, Any]:
        mt5 = self._require()
        tick = mt5.symbol_info_tick(self.symbol)
        if tick is None:
    ...
    def send_pending(self, signal: Signal, volume: float) -> dict[str, Any]:
        mt5 = self._require()
        request = self._pending_request(signal, volume)
        check = mt5.order_check(request)
        result = mt5.order_send(request)
