...
const CHART_PT: Record<string, string> = {
  impulse_up: 'Impulso altista',
  impulse_down: 'Impulso baixista',
  pullback_up: 'Pullback de alta',
  pullback_down: 'Pullback de baixa',
  consolidation: 'Consolidação estreita',
  breakout: 'Rompimento',
  reversal: 'Reversão',
  indecision: 'Indecisão',
}

const SIDE_PT: Record<string, string> = {
  BUY: 'Compra',
  SELL: 'Venda',
  HOLD: 'Não fazer nada',
  FLAT: 'Não fazer nada',
...
export function SessionDashboard({ snap, emptyHint }: { snap: LiveSnap; emptyHint: string }) {
  ...
  const candles = useMemo(() => {
    const rows = snap.candles.map((row) => ({
      ...row,
      label: row.t.replace('T', ' ').slice(11, 16),
    }))
    const mark = snap.quote?.last
    const tick = snap.last_tick
    if (mark && tick && rows.length) {
      const label = tick.replace('T', ' ').slice(11, 19)
      const last = rows[rows.length - 1]
      return [...rows.slice(0, -1), { ...last, close: mark, t: tick, label }]
    }
    return rows
  }, [snap.candles, snap.quote?.last, snap.last_tick])
      ...
      <section className="relative mx-auto grid max-w-6xl gap-4 px-5 py-2 sm:px-8 lg:grid-cols-3">
        <div className="rounded-2xl border border-border bg-elevated/50 p-5 lg:col-span-1">
          ...
          <div className="mt-3 flex items-center gap-3">
            <SideMark side={snap.signal?.side ?? 'HOLD'} />
          ...
          <p className="mt-2 text-xs uppercase tracking-wide text-muted-foreground">
            ...
            <span className="font-semibold text-foreground">
              {snap.signal?.hit_pct != null ? pct(snap.signal.hit_pct * (snap.signal.hit_pct <= 1 ? 100 : 1)) : '—'}
            </span>
          </p>
              ...
              {snap.signals.length === 0 ? (
                ...
                snap.signals.slice(0, 12).map((s) => (
                  <tr key={`${s.t}-${s.side}-${s.reason}`} className="border-t border-border/70">
                    ...
                    <td className="px-4 py-2 text-muted-foreground">{s.phrase || s.reason}</td>
                  </tr>
                ))
              )}
            </tbody>
