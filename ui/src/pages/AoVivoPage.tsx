...
import { EMPTY_SNAP, clock, dayLabel, isTodayStamp, type LiveSnap } from '@/lib/liveTypes'
...
function journalToSnap(day: string, payload: {
  ...
  const lastOrder = payload.orders.at(-1)
  return {
    ...EMPTY_SNAP,
    source: 'journal',
    last_tick: day,
    ...
    n_wins: wins,
    win_rate: payload.n_trades ? (wins / payload.n_trades) * 100 : 0,
    today_pnl: Number(payload.closed_pnl_day || 0),
    net_pnl: Number(payload.closed_pnl_day || 0),
    initial_bank: Number(payload.balance_open || 0),
    bank: Number(payload.balance || payload.equity || payload.balance_open || 0),
    trades,
    signals: payload.orders.map((row) => ({
      t: row.ts,
      side: row.side,
      entry: Number(row.entry),
      ...
      hit_pct: Number(row.hit_pct),
      phrase: row.phrase,
    })),
    signal: lastOrder
      ? {
          side: lastOrder.side,
          entry: Number(lastOrder.entry),
          stop: Number(lastOrder.stop),
          take: Number(lastOrder.take),
          reason: lastOrder.reason,
          chart_type: lastOrder.chart_type,
          hit_pct: Number(lastOrder.hit_pct),
          phrase: lastOrder.phrase,
        }
      : null,
  }
}
...
export function AoVivoPage() {
  ...
  const [firstReal, setFirstReal] = useState<string | null>(null)
  const [journalSnap, setJournalSnap] = useState<LiveSnap | null>(null)
  ...
  useEffect(() => {
    void apiFetch('/api/journal/days')
      .then((res) => readJson<{ first: string; today: string }>(res))
      .then((json) => setFirstReal(json.first))
      .catch(() => undefined)
  }, [])
  ...
  useEffect(() => {
    ...
    void apiFetch(`/api/journal/${viewDate}`)
      .then((res) => readJson<Parameters<typeof journalToSnap>[1]>(res))
      .then((json) => setJournalSnap(journalToSnap(viewDate, json)))
      .catch(() => setJournalSnap(null))
  }, [viewDate])
  ...
  const shown = viewingToday ? snap : journalSnap ?? snap
    ...
    <div className="relative min-h-dvh">
      ...
      <section className="relative mx-auto max-w-6xl px-5 pb-4 sm:px-8">
        ...
        <p className="mt-2 max-w-3xl text-muted-foreground">
          {status}. 5 min · lote 1 mini a partir de R$ 500 · +1 / R$ 1.000 · {snap.config || 'best_bank_1000'}
      ...
      <SessionDashboard
        ...
        emptyHint={
          !viewingToday
            ? firstReal
              ? `Diário de ${dayLabel(viewDate)}. Só entram dias com operação real no MT5.`
              : 'O histórico de datas começa no primeiro dia em que uma ordem for enviada ao MT5.'
            : wait === 'fora_do_ouro'
